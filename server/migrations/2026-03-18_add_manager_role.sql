...
BEGIN
  IF EXISTS (SELECT 1 FROM pg_type WHERE typname = 'enum_users_role') THEN
    IF NOT EXISTS (
      ...
      ALTER TYPE public.enum_users_role ADD VALUE 'manager';
    END IF;
  END IF;
  ...
  IF EXISTS (SELECT 1 FROM pg_type WHERE typname = 'enum_users_role_old') THEN
    IF NOT EXISTS (
      ...
      FROM pg_enum e
      JOIN pg_type t ON t.oid = e.enumtypid
      WHERE t.typname = 'enum_users_role_old' AND e.enumlabel = 'manager'
    ) THEN
      ALTER TYPE public.enum_users_role_old ADD VALUE 'manager';
    END IF;
  END IF;
END $$;
